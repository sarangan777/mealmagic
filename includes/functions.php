...
function loginUser($email, $password) {
  ...
  try {
    ...
    if ($email === 'admin@mealmagic.lk') {
      // First verify if admin exists
      $stmt = $conn->prepare("SELECT id, name, email, password, role FROM users WHERE email = :email AND role = 'admin'");
      $stmt->bindParam(':email', $email);
      $stmt->execute();
      ...
      // For debugging, log the password verification attempt
      error_log("Admin login attempt - Hash in DB: " . $user['password']);
...
function getMostOrderedItems($limit = 5) {
  ...
  $stmt = $conn->prepare("SELECT f.id, f.name, f.image, SUM(oi.quantity) as total_ordered
                         FROM order_items oi
                         JOIN food_items f ON oi.food_item_id = f.id
                         GROUP BY f.id
